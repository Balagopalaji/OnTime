...
The host is macOS; no Windows/Office/.NET publish, Electron packaging, installer, or live PowerPoint execution was performed. Current source/unit/guardrail evidence establishes source conformance, but **real COM emission, packaged helper/app content, restart/orphan behavior, clean install/upgrade/uninstall, and live two-consumer behavior remain pending Stage 7 evidence**. This is not a source-not-built classification.
...
| Scenario | H | Status | Evidence |
|---|---|---|---|
| **S-030** clean Windows install + launch + poll, no extra runtime install | H6 | ✅ Implemented at source; Windows runtime evidence pending | `apps/ppt-timer/electron-builder.yml` declares Windows x64 NSIS output and packages exactly `resources/bin/ppt-probe.exe`. `packages/ppt-bridge/native/windows-ppt-probe/ppt-probe.csproj` targets `net10.0-windows`; `build-windows.ps1` publishes `win-x64` self-contained, single-file, untrimmed with `IncludeNativeLibrariesForSelfExtract=true`. `.github/workflows/ppt-timer-build.yml` builds, rejects loose DLLs, packages, and verifies the payload. A clean Windows install/launch/poll without a preinstalled runtime has not run: **Stage 7 pending**. |
...
| Installer: Windows x64, stable app identity across versions, fresh install + in-place upgrade + settings-preserving uninstall | H6 | ✅ Implemented at source; Windows runtime evidence pending | `apps/ppt-timer/electron-builder.yml` declares `appId: com.ontime.ppttimer`, x64 NSIS and `deleteAppDataOnUninstall: false`; `ppt-timer-build.yml` defines the installer/package verification path. See S-030…S-032 for the still-pending Stage 7 executions. |
...
- **Feature-level remaining gates (not source defects):** Stage 7 still requires Windows/Office/runtime/installer execution, including S-030/S-031/S-032; the S-033 destination remains unset under OQ-1, so its CTA remains securely hidden. Stage 8 signing is also pending. H5 and H6 source are source-accepted.
...
- **Unreconciled / pending execution:**
  - **Implemented at source; Stage 7 evidence pending:** S-030/S-031/S-032. The NSIS identity, self-contained helper/payload, package verification, and data-preserving uninstall policy exist in current source; clean install, upgrade, uninstall, real package contents, and launch/poll still require Windows execution.
