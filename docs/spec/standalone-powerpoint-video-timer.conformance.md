...
| **S-011** >1 video → "N videos" + stable visible headline | H5 | ✅ | `view.ts:videoLabel` → `${videoCount} videos` when `multipleVideos`; `focus-tracker.ts` selects the most-recently-started still-playing video with deterministic fallback for the visible headline. The helper `primaryVideoId` remains raw canonical/diagnostic metadata and is not replaced by the standalone tracker. `focus-tracker.test.ts` and `view.test.ts` "S-011". |
