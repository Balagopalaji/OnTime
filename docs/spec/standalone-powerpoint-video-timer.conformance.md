...
| **S-031** in-place upgrade with same app identity preserves settings | H6 | ✅ Implemented at source; Downstage runtime evidence pending | `electron-builder.yml` has stable Downstage `appId: com.downstage.ppttimer`, package-version-derived installer naming, and no auto-update; that identity is the declared upgrade path for Downstage builds. The retired OnTime beta is intentionally a separate installation identity and is not an in-place predecessor. `settings-schema.ts`/`settings-store.ts` retain the persisted settings surface. A newer-Downstage-installer upgrade with saved settings remains **Stage 7 pending**. |
...
| Installer: Windows x64, stable app identity across versions, fresh install + in-place upgrade + settings-preserving uninstall | H6 | ✅ Implemented at source; Downstage lifecycle acceptance pending | `apps/ppt-timer/electron-builder.yml` declares `appId: com.downstage.ppttimer`, x64 NSIS and `deleteAppDataOnUninstall: false`. Historical OnTime and provisional-AppX lifecycle evidence is recorded in the Stage 7 handoff; the new Downstage identity must receive its own fresh install, upgrade and uninstall acceptance. |
