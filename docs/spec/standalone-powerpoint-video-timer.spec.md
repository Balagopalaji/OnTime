...
- **One canonical PowerPoint capability.** The standalone app and Companion consume the same native STA COM helper and the same established behavior constants already in production: 1-second poll cadence, 600 ms identity-change debounce (timing changes for an already-announced identity commit immediately), two-consecutive-no-video-poll clearing before reporting no video, `>200 ms` elapsed-delta playing inference when explicit status is absent, and a 250 ms end-inference threshold. The standalone must not define a separate flicker, debounce, or clearing policy.
...
- **No local extrapolation.** The standalone never advances remaining/elapsed time between polls on its own; it always displays the most recently observed measurement (S-017).
...
- **S-007** — Given the canonical selected video status is "playing", when the app renders, then it shows a playing badge with the current remaining-or-elapsed time per the active timing-mode setting.
- **S-008** — Given the canonical selected video status is "paused", when the app renders, then it shows a paused badge with the last validated remaining/elapsed value frozen (not advancing).
...
- **S-011** — Given more than one video is present on the current slide, when the app renders, then it shows an "N videos" indicator, and displayed timing is that of the helper's explicit primary-video selection (not a standalone-chosen video).
...
- **S-017** — Given the app has just displayed a measurement from the most recent poll, when more than one second elapses before the next poll result arrives, then the displayed value does not change locally; it only updates when the next poll result is received.
...
- **S-021** — Given the user has moved the window, changed display, resized/selected a preset, toggled always-on-top, or changed timing mode, when the app is closed and relaunched, then all of those settings are restored exactly as last set.
...
- **In-window controls**: always-on-top on/off toggle; remaining-vs-elapsed timing-mode toggle.
- **Persisted settings fields**: window bounds, selected display ID, size preset, always-on-top (boolean), timing mode (remaining/elapsed), schema version.
