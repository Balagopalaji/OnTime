...
Product roadmap: `docs/plans/powerpoint-capability-and-display-roadmap-2026-08-07.md`. Historical product draft: `docs/phase-3-standalone-ppt-timer.md`.
...
- **S-011** — Given more than one video is present on the current slide, when the app renders, then it shows an "N videos" indicator and the visible headline follows the most-recently-started still-playing video with deterministic fallback. The helper primary identity remains raw diagnostic/canonical metadata; the standalone focus tracker selects the visible headline and does not rewrite that helper identity.
