...
**State on `main` (HEAD = #126):**
- God-files: `frontend/src/context/UnifiedDataContext.tsx` **6,067** / `companion/src/main.ts` **6,134**. Stage-1b exit targets `≤5,200` / `≤6,600` — **main.ts now MEETS its target**; UnifiedDataContext still needs ~870 lines (Lane A). D5 finish line = both deleted down to ≤500-line pure-wiring shims.
- Packages 6/10 populated (unchanged this tranche — see the CJS constraint below).
- Lane B #121–#126 merged: B-1 LiveCue/envelope type adoption, B-2/B-4 characterizations (28 tests C1–C16 + D1–D12, every one reviewer-mutation-verified), B-3 pure-helper carve, B-5a probe-I/O + debug-log carve. Ledger synced in #125. `main` clean, **no open PRs, no baton waiting. The 9th milestone audit has NOT run yet** — it is the next orchestrator's first substantive act (see Next tranche).
...
1. **9th milestone audit FIRST** — fresh-context `model:"fable"` over `git diff ba8793a..main` (base = #120, the 8th-audit endpoint state). Audit flags already recorded for it: the B-3/B-5a staging deviation (package population did not advance despite the ratified `presentation-core` home); `pptBackgroundSince` write-only + `PPT_BACKGROUND_CLEAR_MS` unused (dead logic, deliberately untouched — ruling wanted: delete in a fast-lane slice or keep); the D6 warm-cache discovery (two-poll no-video clear unreachable on the warm path — as-is behavior, pinned); byte-faithfulness of B-3 (pure verbatim) and B-5a (verbatim + 26 enumerated DI lines); the B-5a >400-line `ppt-probe.ts` review-approval.
2. **Lane B — B-5b, the last presentation slice:** candidate/commit machine + `handlePowerPointStatus` + `pptVideoCache`/no-video counters + poll loop out of `main.ts` → `companion/src/presentation-candidate.ts` (staged, `// rebuild-target: packages/presentation-core`), injected `{ emitters, getPresentationRoomIds, getCompanionCapabilities }` (controlAuditDeps pattern). Fully characterized already (C7–C16 + D1–D12). CRITICAL: the tests import `handlePowerPointStatus`/`commitPresentationSnapshot`/`updatePresentationCandidate`/`buildPowerPointCue` from `./main.js` — keep the re-export shim (L-B) and do NOT edit the tests. Note the machine's module state is shared with `handlePowerPointStatus` — that is WHY they move together (splitting them requires getter indirection; already analyzed and rejected).
3. **Lane A — localStorage persistence cluster** (`readRoomCache`/`persistRoomCache`, tombstones, subscriptions in `UnifiedDataContext.tsx`) — NOT a verbatim move; needs an **injected storage-adapter** pattern (mirror U7). Frontend CAN value-import packages (Vite), so this one genuinely lands in `local-sync-arbitration` and advances package population. Good candidate for the orchestrator-sub-agent experiment below.
4. **Lane A — stateful arbitration blocks** (`mergeProgressFromCache`, the `ARBITRATION_FLAGS.room` decision paths) — characterize-first; the M5 fresh-wins `mergeProgress` contract is the extraction source of truth.
5. **Fast-lane U8** — wire/mark the zero-caller predicates; optionally the dead `pptBackgroundSince`/`PPT_BACKGROUND_CLEAR_MS` removal if the audit rules to delete.
