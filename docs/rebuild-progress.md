...
**State on `main` (HEAD = #128):**
- God-files: `frontend/src/context/UnifiedDataContext.tsx` **6,067** / `companion/src/main.ts` **5,841**. Stage-1b exit targets `≤5,200` / `≤6,600` — **main.ts is well under its target** (7,387 at tranche start); UnifiedDataContext still needs ~870 lines (Lane A). D5 finish line = both deleted down to ≤500-line pure-wiring shims.
...
2. **Lane A — localStorage persistence cluster** (`readRoomCache`/`persistRoomCache`, tombstones, subscriptions in `UnifiedDataContext.tsx`) — NOT a verbatim move; needs an **injected storage-adapter** pattern (mirror U7). Frontend CAN value-import packages (Vite), so this one genuinely lands in `local-sync-arbitration` and advances package population. Good candidate for the orchestrator-sub-agent experiment below.
...
4. **Fast-lane U8** — wire/mark the zero-caller predicates; optionally the dead `pptBackgroundSince`/`PPT_BACKGROUND_CLEAR_MS` removal if the audit rules to delete.
