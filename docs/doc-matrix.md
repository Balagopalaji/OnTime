...
| Companion controller lock + takeover | `docs/local-server-prd.md` | PRD | planned | `companion/src/main.ts` (planned) |  |  |  |
...
| PowerPoint video timing debug notes | `docs/ppt-video-debug.md` | Reference | current | `companion/src/main.ts` |  |  |  |
