...
- `token`: 6-digit PIN generated by Companion on startup
...
- All `JOIN_ROOM` requests must include valid 6-digit PIN
- Token regenerates on Companion restart
- Displayed in Companion system tray icon menu
...
- Only controller clients can send `TIMER_ACTION` (viewer clients rejected)
- Room ownership verified via Firebase before allowing control
