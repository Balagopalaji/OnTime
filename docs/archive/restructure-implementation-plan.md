...
- **Status: current**: Denotes the intended **implemented** scope. Verification is tracked in the doc-matrix columns (`Owner Verified`, `Verified Date`).

## Phased Approach

### Phase 0: Inventory & Reality Check
Audit existing documentation to distinguish between "implemented" reality and "future" plans. **Exclude all documents in the archive directory from this inventory.**
...
- **Archive Reference Cleanup (Phase 1)**: Remove references to the archive directory within the new `docs/README.md`.
...
- **Lifecycle (Status)**:
    - **current**: Features are expected to be implemented; confirm via doc-matrix verification columns.
...
- **Archive Reference Cleanup (Phase 5)**: Remove all references to the archive directory from these root files.

## Verification Plan
1. **Reality Audit**: All features marked `Status: current` must correspond to existing code and be confirmed via doc-matrix verification columns.
2. **Link integrity**: No broken internal links; index and doc-matrix are mutually linked.
3. **Archive Exclusion**: **No non-archived document (regardless of Type) should contain references or links to the archive directory.**
