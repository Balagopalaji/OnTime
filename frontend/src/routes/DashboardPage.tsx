...
export const DashboardPage = () => {
  ...
  const myRooms = useMemo(() => {
    if (!user) return []
    const owned = rooms.filter((room) => room.ownerId === user.uid && !pendingRooms.has(room.id))
    const sorted = [...owned]
    sorted.sort((a, b) => {
    ...
    return sorted
  }, [pendingRooms, rooms, sortBy, user])
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="space-y-4">
        {myRooms.length === 0 && visiblePlaceholders.length === 0 ? (
                            ...
                            onClick={() => {
                              ...
                              if (navigator.share) {
                                ...
                                void navigator.clipboard
                                  .writeText(viewerUrl)
                                  .then(() => window.alert('Viewer link copied to clipboard'))
                              }
                            }}
